...
public:
    ...
    static void inDanhSachThuoc(const vector<DMThuoc>& ds) {
        ...
        // Độ rộng cột
        const int w1 = 10;  // MaThuoc
        const int w2 = 45;  // TenThuoc
        const int w3 = 15;  // DonViTinh
        const int w4 = 9;   // GiaBan
        const int w5 = 70;  // CongDung
        ...
        for (size_t i = 0; i < ds.size(); i++) {
            if (!ds[i].f) {
                cout << "|" << left << setw(w1) << ds[i].maThuoc
                     << "|" << setw(w2) << ds[i].tenThuoc
                     << "|" << setw(w3) << ds[i].donViTinh
                     << "|" << setw(w4) << ds[i].giaBan
                     << "|" << setw(w5) << ds[i].congDung << "|" << endl;
